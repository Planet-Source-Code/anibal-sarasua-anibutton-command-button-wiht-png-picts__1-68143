...
Begin VB.UserControl AButton 
   ...
   PropertyPages   =   "AButton.ctx":0000
   ScaleHeight     =   240
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   320
   ToolboxBitmap   =   "AButton.ctx":0040
...
Private Const cbVersion As String = "2.1.0" '-> Anibal Sarasua's MOD
...
Private Sub UserControl_InitProperties()
    ...
    ForeO = ForeC
    MaskC = &HC0C0C0
    'Call picPng.LoadPicture_File(App.Path & "\" & "test.png")
    Call CalcTextRects
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    With PropBag
        ...
        Call .WriteProperty("PICN", picNormal, Nothing)
        Call .WriteProperty("PICH", picHover, Nothing)
        ...
        Call .WriteProperty("PATHICON", m_IconPath)
        Test = picPng.SaveToStream(mDatos())
        Call .WriteProperty("PICPNG", mDatos())
        Call .WriteProperty("GCOLOR1", mColor1)
        Call .WriteProperty("GCOLOR2", mColor2)
        Call .WriteProperty("GRADDIR", mGradirection)
    End With
    ...
    With UserControl
        ...
        If isEnabled Then
            If curStat = 0 Then
                ...
                Case 9 'Office XP
                    If IsOver Then
                        'Call FillGradient(UserControl.hDC, 0, 0, UserControl.ScaleWidth, UserControl.ScaleHeight, mColor1, mColor2, Fill_Vertical, False)
                        DrawRectangle 1, 1, Wi, He, OXPf
                        Call DrawCaption(Abs(IsOver))
                        DrawRectangle 0, 0, Wi, He, OXPb, True
                        Call DrawFocusR
                     Else
                        Call FillGradient(UserControl.hDC, 0, 0, UserControl.ScaleWidth, UserControl.ScaleHeight, mColor2, mColor1, mGradirection, False)
                        Call DrawCaption(Abs(IsOver))
                        Call DrawFocusR
                     End If
                Case 11 'transparent
...
Private Sub DrawPictures(ByVal State As Byte)
    ...
    If picPng Is Nothing Then Exit Sub 'check if there is a main picture, if not then exit

    With UserControl
        Select Case State
        Case 0 'normal & hover
            If Not IsOver Then
                picPng.Render .hDC, picPT.X, picPT.Y, , , 70
            Else
                If MyButtonType = [Office XP] Then
                    picPng.Render .hDC, picPT.X + 1, picPT.Y + 1, , , 30
                    picPng.Render .hDC, picPT.X - 1, picPT.Y - 1
                Else
                    If Not picHover Is Nothing Then
                        picPng.Render .hDC, picPT.X, picPT.Y
                    Else
                        picPng.Render .hDC, picPT.X, picPT.Y
                    End If
                End If
        ...
        Case 1 'down
            If picHover Is Nothing Or MyButtonType = [Office XP] Then
                Select Case MyButtonType
                Case 5, 9
                    picPng.Render .hDC, picPT.X + 1, picPT.Y + 1, , , 80
...
Private Sub CalcPicSize()
    ...
    If Not picPng Is Nothing Then
        picSZ.X = picPng.Width 'UserControl.ScaleX(picNormal.Width, 8, UserControl.ScaleMode)
        picSZ.Y = picPng.Height 'UserControl.ScaleY(picPng.Height, 8, UserControl.ScaleMode)
    Else
        picSZ.X = 0: picSZ.Y = 0
    End If
...
'exit if there's no picture
    ...
    If picPng Is Nothing And picHover Is Nothing Then Exit Sub
